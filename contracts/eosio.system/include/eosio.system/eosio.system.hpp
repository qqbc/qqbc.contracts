...
namespace eosiosystem {
   ...
   //voter reward
   static constexpr int64_t  qqbc_reward_initial           = 100;    //10% initial
   static constexpr int64_t  qqbc_reward_inflation         = 2;     //0.2% inflation
   static constexpr int64_t  qqbc_reward_max               = 200;     //20% max reward rate
    ...
    * inviter info.
    *
    * @details Inviter info stores information about the inviter:
    * - `owner` the invitee
    * - `inviters` the inviters approved by this voter
    * - `pending_inviters` the inviters pending for next approval
    */
    struct [[eosio::table, eosio::contract("eosio.system")]] inviter_info {
        name                owner;     /// the voter
        std::vector<name>   inviters;  /// the inviters approved by this voter
        std::vector<name>   pending_inviters; /// the inviters pending for next approval

        uint64_t primary_key()const { return owner.value; }

        // explicit serialization macro is not necessary, used here only to improve compilation time
        EOSLIB_SERIALIZE( inviter_info, (owner)(inviters)(pending_inviters))
    };
   ...
   struct [[eosio::table, eosio::contract("eosio.system")]] voter_info {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      EOSLIB_SERIALIZE( voter_info, (owner)(proxy)(producers)(staked)(last_vote_weight)(proxied_vote_weight)(is_proxy)(flags1)(reserved2)(reserved3)(last_vote_time)(last_staked) )
   };
   ...
   typedef eosio::multi_index< "voters"_n, voter_info >  voters_table;
     ...
     * Inviter table
     *
     * @details The inviters table stores all the `inviter_info`s instances, all inviters information.
     */
    typedef eosio::multi_index< "inviters"_n, inviter_info >  inviters_table;
   ...
   class [[eosio::contract("eosio.system")]] system_contract : public native {
      ...
      public:
         ...
         [[eosio::action]]
         void invite( const name& inviter, const std::vector<name>& invitees );
         ...
         using invite_action = eosio::action_wrapper<"invite"_n, &system_contract::invite>;
      private:
         ...
         //defined in invite.cpp
         void qqbc_reward( const voters_table::const_iterator& voter,
                 const name& voter_name, bool isNewVote,
                 const eosio::time_point&, int64_t last_staked );
         void add_inviter(const name& invitee, const name& inviter);
