   ...
   void system_contract::update_votes( const name& voter_name, const name& proxy, const std::vector<name>& producers, bool voting ) {
      ...
      if( voter->last_vote_weight <= 0.0 ) {
          ...
          if (_gstate.total_activated_stake >= min_activated_stake &&
              ...
              _gstate.thresh_activated_stake_time = ct;
          }
      ...
      //qqbc
      //update the last_staked value if the request is from voteproducers prods
      int64_t last_staked = voter->last_staked;
      if(voting){
          last_staked = voter->staked;
      }

       //no reward if the main net is inactive
      ...
      _voters.modify( voter, same_payer, [&]( auto& av ) {
         ...
         av.proxy     = proxy;
         av.last_vote_time = ct;
         av.last_staked = last_staked;
      });
   }

   void system_contract::qqbc_reward(const voters_table::const_iterator& voter,
           const name& voter_name, bool isNewVote,
           const eosio::time_point& ct, int64_t last_staked){
       inviters_table     inviter_tbl( get_self(), voter_name.value );
       auto iter = inviter_tbl.find(voter_name.value );

       //no reward for the first vote
       if (!isNewVote) {
           //the reward will be computed only after the main net active time
           ...
           if (week > 0){
               ...
               //each inviter get 10% of the voters interest
               ...
               transfer_act.send(get_self(), voter_name, asset(to_voters, core_symbol()), "pay for voter");

               /*
                *    inviter reward logic
                * */
       ...
       //update inviters list after each vote
       if( iter != inviter_tbl.end() ) {
           inviter_tbl.modify(iter, same_payer, [&](auto &av) {
               av.inviters = av.pending_inviters;
               av.pending_inviters.clear();
           });
       }
   }
